...
cab_data18 <- cab_data18[sample(10000000), ]
...
leaflet(tiles2) %>% 
  addTiles() %>% 
  addPolygons(color = "black", fillOpacity = 0.1, label = labels, popup = zone_count) 


tiles2@data <- data.frame(matrix(unlist(tiles2$data), nrow = 263, byrow = T), stringsAsFactors = FALSE)

tiles2@data %>%
  mutate(zone_count = zone_count)
```
